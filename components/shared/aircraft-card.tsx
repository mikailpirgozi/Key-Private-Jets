...
import { getAircraftImage } from '@/lib/data/aircraft-images'
...
export function AircraftCard({ aircraft }: AircraftCardProps) {
  const aircraftImage = getAircraftImage(aircraft.slug)
  ...
  return (
    <Link href={`/aircraft/${aircraft.slug}`} className="block group">
      <div className="luxury-card p-0 h-full flex flex-col relative overflow-hidden">
        {/* Image Container */}
        <div className="relative h-48 w-full overflow-hidden bg-luxury-black-lighter">
          <Image
            src={aircraftImage.main}
          ...
          />
          {/* Overlay gradient */}
          <div className="absolute inset-0 bg-gradient-to-t from-luxury-black via-luxury-black/20 to-transparent opacity-60 group-hover:opacity-40 transition-opacity duration-300" />
        </div>
